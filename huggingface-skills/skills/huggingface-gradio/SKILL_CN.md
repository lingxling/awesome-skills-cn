...
description: 在 Python 中构建 Gradio Web UI 和演示。在创建或编辑 Gradio 应用、组件、事件监听器、布局或聊天机器人时使用。
---

# Gradio

Gradio 是一个用于构建交互式 Web UI 和 ML 演示的 Python 库。此技能涵盖核心 API、模式和示例。
...
创建一个从 {minimum} 到 {maximum} 的滑块，步长为 {step}。
...
创建一个下拉选择框，可从中选择单个或多个条目（作为输入组件）或显示（作为输出组件）。
...
创建一个文件组件，允许上传一个或多个通用文件（用作输入时）或显示通用文件或下载 URL（作为输出）。
...
如果任务需要对现有组件进行重大自定义或创建 Gradio 中不存在的组件，您可以使用 `gr.HTML` 创建一个。它支持 `html_template`（带有 `${}` JS 表达式和 `{{}}` Handlebars 语法）、`css_template` 用于作用域样式，以及 `js_on_load` 用于交互性 — 其中 `props.value` 更新组件值，`trigger('event_name')` 触发 Gradio 事件。为了重用，子类化 `gr.HTML` 并定义 `api_info()` 以支持 API/MCP。请参阅 [完整指南](https://www.gradio.app/guides/custom-HTML-components)。

以下是展示如何创建和使用这些类型组件的示例：
...
`gradio` CLI 包括 `info` 和 `predict` 命令，用于以编程方式与 Gradio 应用交互。这些对于需要在工作流程中使用 Spaces 的编码代理特别有用。
...
返回一个 JSON 有效载荷，描述所有端点、它们的参数（包含类型和默认值）以及返回值。
...
文件类型参数显示 `"type": "filepath"` 并包含 `"meta": {"_type": "gradio.FileData"}` 的说明 — 这表示文件将上传到远程服务器。
