---
name: xlsx
description: "全面的电子表格创建、编辑和分析，支持公式、格式、数据分析和可视化。当 Claude 需要处理电子表格（.xlsx、.xlsm、.csv、.tsv 等）时，用于：(1) 创建带有公式和格式的新电子表格，(2) 读取或分析数据，(3) 在保留公式的同时修改现有电子表格，(4) 电子表格中的数据分析和可视化，或 (5) 重新计算公式"
...
#### 硬编码的文档要求
- 在旁边的单元格中添加注释（如果在表格末尾）。格式："来源：[系统/文档]，[日期]，[特定引用]，[如果适用的 URL]"
- 示例：
...
用户可能会要求您创建、编辑或分析 .xlsx 文件的内容。对于不同的任务，您有不同的工具和工作流程可用。
...
sheet = wb.active  # 或使用 wb['SheetName'] 选择特定工作表
...
### 使用 openpyxl
- 单元格索引是 1 基于的（row=1, column=1 指的是单元格 A1）
- 使用 `data_only=True` 读取计算值：`load_workbook('file.xlsx', data_only=True)`
- **警告**：如果使用 `data_only=True` 打开并保存，公式会被值替换并永久丢失
