...
description: "通过 Rube MCP (Composio) 自动化 Onedesk 任务。始终先搜索工具以获取当前架构。"
requires:
  mcp: [rube]
---

# 通过 Rube MCP 进行 Onedesk 自动化

通过 Rube MCP 使用 Composio 的 Onedesk 工具包自动化 Onedesk 操作。
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | `RUBE_SEARCH_TOOLS` with Onedesk-specific use case |
| 连接 | `RUBE_MANAGE_CONNECTIONS` with toolkit `onedesk` |
