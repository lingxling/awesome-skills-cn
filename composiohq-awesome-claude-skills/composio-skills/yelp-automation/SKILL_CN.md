...
description: "通过 Rube MCP (Composio) 自动化 Yelp 任务。始终先搜索工具以获取当前架构。"
requires:
  mcp: [rube]
---

# 通过 Rube MCP 进行 Yelp 自动化

通过 Rube MCP 使用 Composio 的 Yelp 工具包自动化 Yelp 操作。
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | `RUBE_SEARCH_TOOLS` with Yelp-specific use case |
| 连接 | `RUBE_MANAGE_CONNECTIONS` with toolkit `yelp` |
