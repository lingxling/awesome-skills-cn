...
description: "通过 Rube MCP (Composio) 自动化 21risk 任务。始终先搜索工具以获取当前架构。"
requires:
  mcp: [rube]
---
...
- 通过 `RUBE_MANAGE_CONNECTIONS` 使用工具包 `_21risk` 建立活动的 21risk 连接
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | 使用 21risk 特定用例的 `RUBE_SEARCH_TOOLS` |
| 连接 | 使用工具包 `_21risk` 的 `RUBE_MANAGE_CONNECTIONS` |
