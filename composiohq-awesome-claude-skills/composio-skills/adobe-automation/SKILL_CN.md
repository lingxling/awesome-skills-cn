...
description: "通过 Rube MCP (Composio) 自动化 Adobe 任务。始终先搜索工具以获取当前架构。"
requires:
  mcp: [rube]
---
...
通过 Composio 的 Adobe 工具包，经由 Rube MCP 实现 Adobe 操作的自动化。
...
- 通过带有工具包 `adobe` 的 `RUBE_MANAGE_CONNECTIONS` 激活 Adobe 连接
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | 使用 Adobe 特定用例的 `RUBE_SEARCH_TOOLS` |
| 连接 | 使用工具包 `adobe` 的 `RUBE_MANAGE_CONNECTIONS` |
