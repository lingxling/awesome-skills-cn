...
description: "通过 Rube MCP (Composio) 自动化 Abuselpdb 任务。始终先搜索工具以获取当前架构。"
requires:
  mcp: [rube]
---

# 通过 Rube MCP 进行 Abuselpdb 自动化

通过 Rube MCP 使用 Composio 的 Abuselpdb 工具包自动化 Abuselpdb 操作。

**工具包文档**: [composio.dev/toolkits/abuselpdb](https://composio.dev/toolkits/abuselpdb)

## 前置条件

- Rube MCP 必须已连接（RUBE_SEARCH_TOOLS 可用）
- 通过 `RUBE_MANAGE_CONNECTIONS` 使用工具包 `abuselpdb` 建立活动的 Abuselpdb 连接
...
2. 使用工具包 `abuselpdb` 调用 `RUBE_MANAGE_CONNECTIONS`
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | 使用 Abuselpdb 特定用例的 `RUBE_SEARCH_TOOLS` |
| 连接 | 使用工具包 `abuselpdb` 的 `RUBE_MANAGE_CONNECTIONS` |
