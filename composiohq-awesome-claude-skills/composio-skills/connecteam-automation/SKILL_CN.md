...
description: "通过 Rube MCP (Composio) 自动化 连接eam 任务。始终先搜索工具以获取当前架构。"
requires:
  mcp: [rube]
---

# 通过 Rube MCP 进行 连接eam 自动化

通过 Rube MCP 使用 Composio 的 连接eam 工具包自动化 连接eam 操作。
...
- Active 连接eam connection via `RUBE_MANAGE_CONNECTIONS` with toolkit `connecteam`
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "连接eam operations", known_fields: ""}]
...
```
RUBE_SEARCH_TOOLS
queries: [{use_case: "your specific 连接eam task"}]
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | `RUBE_SEARCH_TOOLS` with 连接eam-specific use case |
| 连接 | `RUBE_MANAGE_CONNECTIONS` with toolkit `connecteam` |
