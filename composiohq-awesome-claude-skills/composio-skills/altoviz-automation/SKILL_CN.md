...
description: "通过 Rube MCP (Composio) 自动化 Altoviz 任务。始终先搜索工具以获取当前架构。"
requires:
  mcp: [rube]
---

# 通过 Rube MCP 进行 Altoviz 自动化
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | `RUBE_SEARCH_TOOLS` with Altoviz-specific use case |
| 连接 | `RUBE_MANAGE_CONNECTIONS` with toolkit `altoviz` |
