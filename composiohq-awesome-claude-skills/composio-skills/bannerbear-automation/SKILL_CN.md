...
description: "通过 Rube MCP (Composio) 自动化 Bannerbear 任务。始终先搜索工具以获取当前架构。"
requires:
  mcp: [rube]
---

# 通过 Rube MCP 进行 Bannerbear 自动化

通过 Rube MCP 使用 Composio 的 Bannerbear 工具包自动化 Bannerbear 操作。
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | `RUBE_SEARCH_TOOLS` with Bannerbear-specific use case |
| 连接 | `RUBE_MANAGE_CONNECTIONS` with toolkit `bannerbear` |
