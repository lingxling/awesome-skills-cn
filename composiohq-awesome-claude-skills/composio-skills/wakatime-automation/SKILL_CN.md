...
description: "通过 Rube MCP (Composio) 自动化 Wakatime 任务。始终先搜索工具以获取当前架构。"
requires:
  mcp: [rube]
---

# 通过 Rube MCP 进行 Wakatime 自动化

通过 Rube MCP 使用 Composio 的 Wakatime 工具包自动化 Wakatime 操作。
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | `RUBE_SEARCH_TOOLS` with Wakatime-specific use case |
| 连接 | `RUBE_MANAGE_CONNECTIONS` with toolkit `wakatime` |
