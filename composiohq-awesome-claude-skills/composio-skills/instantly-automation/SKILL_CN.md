...
1. Add the Composio MCP server to your client configuration:
   ...
   https://rube.app/mcp
   ```
2. 连接 your Instantly account when prompted (API key authentication).
...
**Key parameters:**
- `status` -- Filter by status: `1` (Active), `2` (Paused), `-1` (连接ion Error), `-2` (Soft Bounce Error), `-3` (Sending Error)
