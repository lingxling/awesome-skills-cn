...
description: "通过 Rube MCP (Composio) 自动化 Abstract 任务。始终先搜索工具以获取当前架构。"
requires:
  mcp: [rube]
---

# 通过 Rube MCP 进行 Abstract 自动化

通过 Rube MCP 使用 Composio 的 Abstract 工具包自动化 Abstract 操作。

**工具包文档**: [composio.dev/toolkits/abstract](https://composio.dev/toolkits/abstract)

## 前置条件

- Rube MCP 必须已连接（RUBE_SEARCH_TOOLS 可用）
- 通过 `RUBE_MANAGE_CONNECTIONS` 使用工具包 `abstract` 建立活动的 Abstract 连接
...
2. 使用工具包 `abstract` 调用 `RUBE_MANAGE_CONNECTIONS`
...
| 操作 | 方法 |
|-----------|----------|
| 查找工具 | 使用 Abstract 特定用例的 `RUBE_SEARCH_TOOLS` |
| 连接 | 使用工具包 `abstract` 的 `RUBE_MANAGE_CONNECTIONS` |
