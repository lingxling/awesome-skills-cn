...
description: "使用 Figma MCP 工作流（设计上下文、截图、资产和项目约定转换）将 Figma 设计转换为生产就绪的代码，实现像素完美的准确性。当用户提供 Figma URL 或节点 ID，或要求实现必须匹配 Figma 规范的设计或组件时触发。需要工作的 Figma MCP 服务器连接。"
---
...
成功登录后，用户需要重启 codex。您应该完成回答并告诉他们，以便再次尝试时可以继续步骤 1。
...
**设计系统集成：**
- 始终在可能时使用项目设计系统中的组件。
- 将 Figma 设计令牌映射到项目设计令牌（例如，`primary-500`、`primary-hover`）。
...
2. 运行 `get_design_context(fileKey="kL9xQn2VwM8pYrTb4ZcHjF", nodeId="42-15")` 获取设计上下文
3. 运行 `get_screenshot(fileKey="kL9xQn2VwM8pYrTb4ZcHjF", nodeId="42-15")` 获取视觉参考
4. 从资产端点下载任何按钮图标
5. 检查项目是否有现有的按钮组件
6. 如果是，使用新变体扩展它；如果否，使用项目约定创建新组件
7. 将 Figma 颜色映射到项目设计令牌（例如，`primary-500`、`primary-hover`）
8. 使用 Tailwind 实用类或项目令牌实现按钮
9. 针对截图验证填充、边框半径、排版
10. 如果是，扩展它以包含新变体；如果否，使用项目约定创建新组件
...
5. 运行 `get_screenshot(fileKey="pR8mNv5KqXzGwY2JtCfL4D", nodeId="10-5")` 获取完整页面视觉参考
6. 下载所有资产（徽标、图标、图表）
7. 使用项目的布局原语构建布局
8. 使用现有组件实现每个部分（页眉、侧边栏、内容卡片）
9. 验证响应式行为是否符合 Figma 约束
10. 针对完整页面截图验证布局、间距、颜色和排版

**结果：** 完全匹配 Figma 设计的响应式仪表板，使用项目组件。
...
Figma 实现工作流程为设计师和开发者建立了可靠的过程：

**对于设计师：**
- 确保实现将匹配他们的设计，具有像素完美的准确性。
- 使用设计系统和组件作为单一事实来源。

**对于开发者：**
- 结构化的方法消除了猜测工作并减少了返工。
- 一致的、高质量的实现，保持设计系统完整性。

**对于团队：**
- 清晰的流程减少误解。
- 1:1 的视觉保真度确保产品符合设计规范。
