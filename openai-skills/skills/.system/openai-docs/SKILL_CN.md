...
使用 developers.openai.com MCP 服务器从 OpenAI 开发者文档提供权威、最新的指导。始终优先考虑开发者文档 MCP 工具而不是 web.run 来处理 OpenAI 相关问题。此技能还可以从 `references/` 加载针对模型选择、模型升级和提示升级请求的目标文件，但当前 OpenAI 文档仍然是权威的。仅当 MCP 服务器已安装且没有返回有意义的结果时，您才应该回退到 Web 搜索。
...
- 对于模型升级或提示升级，当脚本存在时从此技能目录运行 `node scripts/resolve-latest-model-info.js`，然后按照 `references/upgrade-guide.md` 行事，除非解析器为动态的最新/当前/默认请求返回更新的指导。
