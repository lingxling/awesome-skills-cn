# Claude Scientific Skills 中文翻译

> 本文档是 [Claude Scientific Skills](https://github.com/K-Dense-AI/claude-scientific-skills) 项目的中文翻译版本。

## 关于本翻译

### 原项目介绍

**Claude Scientific Skills** 是由 [K-Dense](https://k-dense.ai) 创建的综合性科学技能集合，包含 **170+ 即用型科学和研究技能**（现已涵盖癌症基因组学、药物-靶点结合、分子动力学、RNA 速度、地理空间科学、时间序列预测、FRED 经济数据等），适用于任何支持开放的 [Agent Skills](https://agentskills.io/) 标准的 AI 代理。可与 **Cursor、Claude Code、Codex 等工具**配合使用。将您的 AI 代理转变为能够跨生物学、化学、医学等领域执行复杂多步骤科学工作流的研究助手。
...
- [scientific-skills/](scientific-skills/) - 170+ 科学和研究技能，涵盖生物信息学、化学信息学、临床研究、机器学习、数据可视化等多个领域
...
一个综合性集合，包含 **170+ 即用型科学和研究技能**（现已涵盖癌症基因组学、药物-靶点结合、分子动力学、RNA 速度、地理空间科学、时间序列预测、FRED 经济数据等），适用于任何支持开放的 [Agent Skills](https://agentskills.io/) 标准的 AI 代理，由 [K-Dense](https://k-dense.ai) 创建。可与 **Cursor、Claude Code、Codex 等工具**配合使用。将您的 AI 代理转变为能够跨生物学、化学、医学等领域执行复杂多步骤科学工作流的研究助手。
...
<p align="center">
  ...
  <br/>
  <em>上面的演示展示了 <a href="https://k-dense.ai">K-Dense Web</a> —— 建立在这些技能之上的托管平台。Claude Scientific Skills 是开源技能集合；K-Dense Web 是功能更强大、零设置的完整 AI 共同科学家平台。</em>
</p>
...
> 🎬 **Claude Scientific Skills 新手？** 观看我们的 [Claude Scientific Skills 入门](https://youtu.be/ZxbnDaD_FVg) 视频以快速了解。

---

## 📦 包含内容

本仓库提供 **170 个科学和研究技能**，组织成以下类别：

- **250+ 科学与金融数据库** - 总的来说，这些技能提供对 250 多个数据库和数据源的访问。专用技能涵盖 PubMed、ChEMBL、UniProt、COSMIC、ClinicalTrials.gov、SEC EDGAR、Alpha Vantage 等；多数据库包如 BioServices（约 40 个生物信息学服务 + 30+ PSICQUIC 交互数据库）、BioPython（通过 Entrez 访问 38 个 NCBI 子数据库）和 gget（20+ 基因组学数据库）占其余部分
- **60+ 优化的 Python 包技能** - 为 RDKit、Scanpy、PyTorch Lightning、scikit-learn、BioPython、pyzotero、BioServices、PennyLane、Qiskit、OpenMM、MDAnalysis、scVelo、TimesFM 等明确定义的技能 —— 包含精心策划的文档、示例和最佳实践。注意：代理可以使用 *任何* Python 包编写代码，不仅仅是这些；这些技能只是为列出的包提供更强、更可靠的性能
- **15+ 科学集成技能** - 为 Benchling、DNAnexus、LatchBio、OMERO、Protocols.io 等明确定义的技能。同样，代理不限于这些 —— Python 可以访问的任何 API 或平台都可以使用；这些技能是优化的、预先记录的路径
- **35+ 分析与交流工具** - 文献综述、科学写作、同行评审、文档处理、海报、幻灯片、示意图、信息图表、Mermaid 图表等
- **10+ 研究与临床工具** - 假设生成、资助写作、临床决策支持、治疗计划、监管合规
...
### 🎯 **全面覆盖**
- **170 个技能** - 覆盖所有主要科学领域
- **250+ 数据库** - 集体访问 250+ 数据库和数据源，涵盖基因组学、化学、临床、金融等 —— 通过专用数据库技能和多数据库包如 BioServices、BioPython 和 gget
- **60+ 优化的 Python 包技能** - RDKit、Scanpy、PyTorch Lightning、scikit-learn、BioServices、PennyLane、Qiskit、OpenMM、scVelo、TimesFM 等（代理可以使用任何 Python 包；这些是预先记录的、更高性能的路径）
...
Claude Scientific Skills 遵循开放的 [Agent Skills](https://agentskills.io/) 标准。只需将技能文件夹复制到您的技能目录，您的 AI 代理就会自动发现并使用它们。
...
将 `scientific-skills/` 中的各个技能文件夹复制到以下支持的技能目录之一。您可以 **全局安装**技能（在所有项目中可用）或 **按项目安装**（仅在该项目中可用）。
...
| 工具 | 目录 |
|------|-----------|
| Cursor | `.cursor/skills/`（在您的项目根目录中） |
| Claude Code | `.claude/skills/`（在您的项目根目录中） |
| Codex | `.codex/skills/`（在您的项目根目录中） |
| Gemini CLI | `.gemini/skills/`（在您的项目根目录中） |

> **注意：** Cursor 也从 `.claude/skills/`、`.codex/skills/` 和 `.gemini/skills/` 目录读取，反之亦然，因此技能在工具之间是交叉兼容的。
...
Claude Scientific Skills 由 **50+ 令人难以置信的开源项目**提供支持，这些项目由全球致力于的开发者和研究社区维护。像 Biopython、Scanpy、RDKit、scikit-learn、PyTorch Lightning 等项目构成了这些技能的基础。
...
*[k-dense.ai](https://k-dense.ai) | [阅读完整比较](https://k-dense.ai/blog/k-dense-web-vs-claude-scientific-skills)*
...
本仓库包含 **170 个科学和研究技能**，组织在多个领域。每个技能为使用科学库、数据库和工具提供综合文档、代码示例和最佳实践。
...
- 基因组学工具：gget、geniml、gtars、deepTools、FlowIO、Zarr、TileDB-VCF
...
- 癌症基因组学：cBioPortal（400+ 研究中的体细胞突变、CNA、表达、生存）、DepMap（癌症依赖性评分、药物敏感性）
- 疾病-基因关联：Monarch Initiative（OMIM、ORPHANET、HPO、ClinVar、模式生物数据）
- 癌症影像：NCI Imaging Data Commons（通过 idc-index 获取放射学和病理学数据集）
...
#### 🔬 **科学数据库**（37+ 专用技能 → 250+ 数据库总计）
> 这 37+ 个技能每个都提供对命名数据库的直接、优化访问。然而，总的来说，这些技能解锁了 **250+ 数据库和数据源** —— 多数据库包如 BioServices（约 40 个生物信息学服务 + 30+ PSICQUIC 交互数据库）、BioPython（通过 Entrez 访问 38 个 NCBI 子数据库）和 gget（20+ 基因组学数据库）增加了此处列出的内容之外的更多覆盖范围。
- 蛋白质：UniProt、PDB、AlphaFold DB、InterPro（蛋白质家族、结构域、Pfam、PANTHER、SMART + 11 个其他）
- 化学：PubChem、ChEMBL、DrugBank、ZINC、HMDB、BindingDB（药物-靶点结合亲和力）
- 基因组学：Ensembl、NCBI Gene、GEO、ENA、GWAS Catalog、gnomAD（人群等位基因频率、pLI/LOEUF）、GTEx（组织特异性表达、eQTL）、JASPAR（转录因子结合位点谱）
- 文献：bioRxiv（预印本）
- 临床：ClinVar、COSMIC、ClinicalTrials.gov、ClinPGx、FDA 数据库、cBioPortal（癌症基因组学）、DepMap（癌细胞系依赖性）、Monarch Initiative（罕见疾病、HPO、跨物种）
...
- 美国联邦财政数据：usfiscaldata（国家债务、每日/每月财政部报表、国债拍卖、利率、汇率、储蓄债券）
- 宏观经济数据：FRED（来自 100+ 来源的 800,000+ 经济时间序列 —— GDP、失业、通货膨胀、住房、通过联邦储备经济数据 API 获取的区域数据）
- 对冲基金系统性风险：hedgefundmonitor（OFR 对冲基金监控 API —— Form PF 汇总统计、CFTC 期货头寸、FICC 赞助回购、SCOOS 交易商融资）
- 全球市场数据：alpha-vantage（通过 Alpha Vantage API 获取实时和历史股票、期权、外汇、加密货币、大宗商品、经济指标、50+ 技术指标）
