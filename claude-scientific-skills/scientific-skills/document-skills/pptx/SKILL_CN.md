---
name: pptx
description: 演示文稿工具包（.pptx）。创建/编辑幻灯片、布局、内容、演讲者备注、注释，用于程序化演示文稿创建和修改。
...
.pptx 文件是一个包含 XML 文件和资源的 ZIP 存档。使用文本提取、原始 XML 访问或 html2pptx 工作流来创建、编辑或分析 PowerPoint 演示文稿。将此技能应用于程序化演示文稿创建和修改。
...
### 原始 XML 访问
原始 XML 访问需要用于：注释、演讲者备注、幻灯片布局、动画、设计元素和复杂格式。对于这些功能中的任何一个，解包演示文稿并读取其原始 XML 内容。

#### 解包文件
`python ooxml/scripts/unpack.py <office_file> <output_dir>`
**注意**：unpack.py 脚本位于 `skills/pptx/ooxml/scripts/unpack.py` 相对于项目根目录。如果此路径不存在脚本，使用 `find . -name "unpack.py"` 来定位它。
...
## 在没有模板的情况下创建新的 PowerPoint 演示文稿

从头创建新的 PowerPoint 演示文稿时，使用 **html2pptx** 工作流将 HTML 幻灯片转换为具有准确定位的 PowerPoint。
...
- **大胆尝试**：尝试意外的组合 - 医疗保健演示文稿不必是绿色的，金融不必是海军蓝的
...
9. **酸橙和李子色**：酸橙（#C5DE82）、李子色（#7C3A5F）、珊瑚色（#FD8C6E）、蓝灰色（#98ACB5）
...
### 布局提示
**对于带有图表或表格的幻灯片：**
- **两列布局（首选）**：使用跨越全宽度的标题，然后下方的两列 - 一列中的文本/项目符号，另一列中的特色内容。这提供了更好的平衡，并使图表/表格更具可读性。使用具有不等列宽的 flexbox（例如，40%/60% 拆分）以优化每种内容类型的空间。
- **全幻灯片布局**：让特色内容（图表/表格）占据整个幻灯片以获得最大影响力和可读性
- **永远不要垂直堆叠**：不要在单列中将图表/表格放置在文本下方 - 这会导致可读性差和布局问题

### 工作流
1. **强制 - 阅读整个文件**：从头到尾完整阅读 [`html2pptx.md`](html2pptx.md)。**阅读此文件时永远不要设置任何范围限制。** 在继续创建演示文稿之前，阅读完整的文件内容以获取详细语法、关键格式规则和最佳实践。
2. 为每张幻灯片创建具有适当尺寸的 HTML 文件（例如，16:9 为 720pt × 405pt）
   - 对所有文本内容使用 `<p>`、`<h1>`-`<h6>`、`<ul>`、`<ol>`
   - 对将添加图表/表格的区域使用 `class="placeholder"`（渲染为灰色背景以获得可见性）
...
要编辑现有 PowerPoint 演示文稿中的幻灯片，使用原始 Office Open XML (OOXML) 格式。这涉及解包 .pptx 文件、编辑 XML 内容并重新打包它。

### 工作流
1. **强制 - 阅读整个文件**：从头到尾完整阅读 [`ooxml.md`](ooxml.md)（约 500 行）。**阅读此文件时永远不要设置任何范围限制。** 在任何演示文稿编辑之前，阅读完整的文件内容以获取有关 OOXML 结构和编辑工作流的详细指导。
...
要创建遵循现有模板设计的演示文稿，在替换占位符上下文之前复制并重新排列模板幻灯片。
...
1. **提取模板文本并创建视觉缩略图网格**：
   ...
   * 请参阅[创建缩略图网格](#creating-thumbnail-grids)部分以获取更多详细信息
...
3. **基于模板清单创建演示文稿大纲**：
   ...
   * **关键：将布局结构与实际内容匹配**：
     ...
     - 图像 + 文本布局：仅当实际有可插入的图像时使用
     - 引用布局：仅用于来自人员的实际引用（带有归属），从不用于强调
     - 永远不要使用占位符多于可用内容的布局
     - 如果有 2 个项目，不要将它们强制放入 3 列布局
     - 如果有 4+ 个项目，考虑拆分为多张幻灯片或使用列表格式
   * 在选择布局之前计算实际内容片段
...
4. **使用 `rearrange.py` 复制、重新排序和删除幻灯片**：
   * 使用 `scripts/rearrange.py` 脚本创建具有所需顺序幻灯片的新演示文稿：
     ...
     ```
   * 该脚本处理重复的幻灯片、删除未使用的幻灯片并自动重新排序
   * 幻灯片索引是基于 0 的（第一张幻灯片是 0，第二张是 1 等）
   * 同一个幻灯片索引可以出现多次以复制该幻灯片
...
5. **使用 `inventory.py` 脚本提取所有文本**：
      ...
      ```json
        {
          "slide-0": {
            "shape-0": {
              "placeholder_type": "TITLE",  // 或 null 用于非占位符
              "left": 1.5,                  // 以英寸为单位的位置
              "top": 2.0,
              "width": 7.5,
              "height": 1.2,
   ...
   * 关键功能：
     ...
     - **间距**：以点为单位的 `space_before`、`space_after` 和 `line_spacing`（仅在设置时包含）
     - **颜色**：用于 RGB 的 `color`（例如，"FF0000"），用于主题颜色的 `theme_color`（例如，"DARK_1"）
     - **属性**：仅输出中包含非默认值
...
6. **生成替换文本并将数据保存到 JSON 文件**
   ...
   - **基本格式规则**：
     ...
     - 列表项应具有 `"bullet": true, "level": 0`（当 bullet 为 true 时需要 level）
     - 保留任何对齐属性（例如，用于居中文本的 `"alignment": "CENTER"`）
     - 在与默认值不同时包括字体属性（例如，`"font_size": 14.0`、`"font_name": "Lora"`）
     - 颜色：对 RGB 使用 `"color": "FF0000"`，对主题颜色使用 `"theme_color": "DARK_1"`
     - 替换脚本期望**正确格式的段落**，而不仅仅是文本字符串
     - **重叠形状**：偏好具有更大 default_font_size 或更适当 placeholder_type 的形状
...
7. **使用 `replace.py` 脚本应用替换**
   ...
   ```
   错误：替换文本使这些形状中的溢出更严重：
     - slide-0/shape-2：溢出恶化了 1.25"（原来为 0.00"，现在为 1.25"）
   ```
...
- 自定义前缀：`python scripts/thumbnail.py template.pptx my-grid`
  - 注意：如果要在特定目录中输出，输出前缀应包括路径（例如，`workspace/my-grid`）
- 调整列数：`--cols 4`（范围：3-6，影响每网格的幻灯片）
- 网格限制：3 列 = 每网格 12 张幻灯片，4 列 = 20，5 列 = 30，6 列 = 42
...
特定范围的示例：
```bash
pdftoppm -jpeg -r 150 -f 2 -l 5 template.pdf slide  # 仅转换 pages 2-5
```
